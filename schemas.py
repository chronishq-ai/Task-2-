...
class Signal(BaseModel):
    ...
    rationale: Optional[str] = Field(
        default=None,
        description=(
            "v0.2 addition: one-sentence reason for the confidence estimate, "
            "e.g. which words in the event drove it. Optional and additive - "
            "Pod A's v0.1 contract (value + confidence only) still parses "
            "fine if this field is ignored downstream."
        ),
    )
